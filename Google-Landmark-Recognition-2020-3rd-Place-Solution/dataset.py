...
def get_df(kernel_type, data_dir, train_step):
    ...
    if train_step == 0:
        ...
        df_train = pd.read_csv(os.path.join(data_dir, 'train.csv')).set_index('landmark_id').loc[
            cls_81313].reset_index()
    ...
    df.to_csv(os.path.join(data_dir, 'tmp', 'train_dataset.csv'), index=False)
